...
<? } ?>
    ...
    <? if ($_list_style == 'thread') { // 타래형 ?>
		<div class="thread-box theme-box<? if ($list[$i]['is_notice']) echo " bo_notice"; ?>">
            ...
            <?
                ...
                $thread_co_sql = "SELECT * FROM (SELECT * FROM {$write_table} WHERE wr_parent = '{$list[$i]['wr_id']}' AND wr_is_comment = '1' ORDER BY wr_comment DESC limit 2) AS sub ORDER BY wr_comment ASC";
	...
	<fieldset id="bo_sch" class="txt-center">
		...
		<button type="submit" class="ui-btn point ico search default"><i class="fas fa-search"></i></button>
		</form>
	</fieldset>
	<!-- } 게시판 검색 끝 -->
</div>
