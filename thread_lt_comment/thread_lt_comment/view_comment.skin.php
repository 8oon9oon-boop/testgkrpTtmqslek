    ...
    <div class="item" id="c_<? echo $comment_id ?>">
        <div class="co_head_wrap">
            ...
            <div class="co-info">
                ...
                <? if($comment_list[$c]['is_edit'] || $comment_list[$c]['is_del']) { ?>
        ...
        <div class="co-content">
            <? if ($comment_list[$c]['wr_1'] == 'hide') { ?>
                <button type="button" class="ui-btn" onclick="toggleComment('<?=$comment_id?>');">펼치기</button>
            <? } ?>
    ...
    <form name="fviewcomment" action="<?=$board_skin_url; ?>/write_reply_update.php" onsubmit="return fviewreply_submit(this);" method="post" autocomplete="off">
        ...
        <div class="board-comment-form">
            ...
            <div class="btn_confirm">
                <button type="submit" id="btn_submit_<?=$list[$i]['wr_id'];?>">등록</button>
            </div>
        </div>
        
        <?php if ($is_guest) { ?>
        <p style="margin-top:5px;">
            <input type="text" name="wr_name" value="<?php echo get_cookie("ck_sns_name"); ?>" required class="frm_input" size="10" placeholder="이름" style="border:1px solid #000; padding:3px;">
            <input type="password" name="wr_password" required class="frm_input" size="10" placeholder="비밀번호" style="border:1px solid #000; padding:3px;">
        </p>
        <?php } ?>
