...
<div class="<?=$cmt_depth>0 ? "item-reply ":"";?><?=($has_child['wr_id']&&($has_child['wr_id']!=$log_comment['wr_id'])) ? "parent " : "";?><?=$has_child['wr_id']==$log_comment['wr_id'] ? "last ": "";?>item-comment" id="c_<?php echo $comment_id ?>" style="padding-left:<?=$cmt_depth?>px;">
	...
	<div class="co-content">
		...
		<?}else if ($log_comment['wr_secret']==1||$log_comment['wr_2']){ //@230715?>
			<span class="co-member highlight">#member only</span> 
		<?} 
			if ($log_comment['c_view']&&$log_comment['wr_reply_more']==1){ //@230715 ?>
			<a href="#" class="co-more highlight"><span>#more</span></a>
			<?} 
		?>
		<div class="original_comment_area<?=($log_comment['c_view']&&$log_comment['wr_reply_more']) ? " re_more":"";?>"> <?//@230715?>
			<?  if($log_comment['c_view']){ //@230715
			...
			<? } 
			// 코멘트 출력 부분
			if($log_comment['wr_type']=="UPLOAD" && $log_comment['wr_is_comment']!=0){
		...
		<div class="modify_area" id="save_comment_<?php echo $comment_id ?>"> 
			...
			<input type="checkbox" id="save_mem_<?php echo $comment_id ?>" name="wr_2" value="1" class="mem" <?if($log_comment['wr_2']) echo "checked";?> />
