	...
	if ($c_is_html || $c_is_secret ) {
		...
		if ($c_is_secret) {
			if ($is_admin || $c_is_secret==1) {
				if(strstr($list_item['wr_option'],'secret')) $c_secret="checked";
				else $c_secret="";
				$c_option .= "\n".'<span><input type="checkbox" class="secret" id="secret_'.$list_item['wr_id'].'" name="secret" value="secret" '.$c_secret.'>'."\n".'<label for="secret_'.$list_item['wr_id'].'">비밀글</label></span>';
			} else {
				$c_option_hidden .= '<input type="hidden" name="secret" value="secret">';
			}
		} 
		if ($is_member) { //@230715
			if($list_item['wr_secret']) $c_mem="checked";
			else $c_mem="";
			$c_option .= "\n".'<span><input type="checkbox" class="mem" id="mem_'.$list_item['wr_id'].'" name="wr_2" value="1" '.$c_mem.'>'."\n".'<label for="mem_'.$list_item['wr_id'].'">멤버공개</label></span>';
		}
	}
